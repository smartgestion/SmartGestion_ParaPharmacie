...
// `bon_commande_id` and are excluded from those calculations.
export function AvoirFournisseurForm({ onSuccess }: AvoirFournisseurFormProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!user?.id) return;
      try {
        const [{ data: fournisseursData }, { data: produitsData }] = await Promise.all([
  ...
  const onSubmit = async (data: AvoirFournisseurFormValues) => {
    ...
    try {
      ...
      const payload = {
        user_id: user?.id,
        numero: avoirNum,
        // Manual avoir: not linked to any purchase order. This is what makes it
        // count against expenses & stock on the Dashboard.
  ...
  const handleProduitSelect = (index: number, produitId: string) => {
    ...
    if (produit) {
      ...
      form.setValue(`lignes.${index}.prixUnitaireHt`, prixAchatTtcProd > 0
        ...
        : Number(produit.prix_achat_ht || produit.prixAchatHt || produit.prix_vente_ht || 0));
